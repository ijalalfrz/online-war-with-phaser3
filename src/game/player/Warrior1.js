...
export default class Warrior1 extends BaseCharacter {
    constructor() {

        super('warrior1', 48, 48, true, 'png')
    ...
    buildAnimation(scene) {
        ...
        this.createAnimation(scene, this.keyWalkUp, this.keyWalkUp, 'walk-up-', 7, 0, 12, -1)
        this.createAnimation(scene, this.keyWalkDown, this.keyWalkDown, 'walk-down-', 7, 0, 12, -1)
        this.createAnimation(scene, this.keyWalkSide, this.keyWalkSide, 'walk-side-', 7, 0, 12, -1)

        this.createAnimation(scene, this.keyAttackUp, this.keyAttackUp, 'attack-up-', 5, 0, 12, -1)
        this.createAnimation(scene, this.keyAttackSide, this.keyAttackSide, 'attack-side-', 5, 0, 12, 0)
        this.createAnimation(scene, this.keyDeath, this.keyDeath, 'death-side-', 4, 0, 12, 1)
    ...
    moveToEnemy(scene, target) {
        ...
        if (distance > 20) {
            ...
            this.isMoving = false
            this.stop()
            console.log(target.character.health)
            console.log(target.alive())
            console.log(this.attackPlay, "--")
            if (!this.attackPlay) {
                console.log('+++')
